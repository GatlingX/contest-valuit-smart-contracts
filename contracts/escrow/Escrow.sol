...
contract Escrow is Ownable, EscrowStorage{
    ...
    function deposit(address _token, uint256 _amount, string memory orderID) external{
    ...
    function settlement(string memory orderID) external{
        require (AgentRole(investorOrders[orderID].asset).isAgent(msg.sender), "Invalid Issuer");

        TransferHelper.safeTransfer(stableCoin, msg.sender, investorOrders[orderID].value - ((investorOrders[orderID].value * adminFee)/100));

        TransferHelper.safeTransfer(stableCoin, owner(), (investorOrders[orderID].value * adminFee)/100);
