...
describe("Hold Time Module testing", function () { let signers: SignerWithAddress[]; let owner: SignerWithAddress; let tokenIssuer: SignerWithAddress; let transferAgent: SignerWithAddress; let user1: SignerWithAddress; let user2: SignerWithAddress; let user3: SignerWithAddress;
...
describe("HoldTimeModule", function () {
    describe("setHoldTime", function () {
        it("should set hold time successfully", async function () {
            ...
            const isBound = await holdTimeCompliance.isComplianceBound(modularComplianceImplementation.address);
            
            // List of countries to be allowed
            const holdTime=Math.floor(Date.now())+1000;
            const encodedData = ethers.utils.defaultAbiCoder.encode(
                ["uint256"], // data types
                [holdTime] // actual data to encode
            );

            const functionSignature = "setHoldTime(uint256)";
            ...
            await modularComplianceImplementation.callModuleFunction(callData,holdTimeCompliance.address);
        });
    });

    describe("getHoldTime", function () {
        it("should return the correct hold time value", async function () {
            // Initialize the modular compliance contract
            await modularComplianceImplementation.init();
            ...
            const encodedData2 = ethers.utils.defaultAbiCoder.encode(
                ["address"], // data types
                [modularComplianceImplementation.address] // actual data to encode
            );
            const functionSignature2 = "getHoldTime(address)";
            const functionSelector2 = ethers.utils.id(functionSignature2).slice(0, 10); // Get the first 4 bytes
            const callData2= functionSelector2+ encodedData2.slice(2); // Remove '0x' prefix from encoded data
            await expect (modularComplianceImplementation.callModuleFunction(callData2,holdTimeCompliance.address));
        });
    });

    describe("isHoldtimeSet", function () {
        it("should return true if hold time is set", async function () {
            ...
            const functionSignature2 = "isHoldtimeSet(address)";
            const functionSelector2 = ethers.utils.id(functionSignature2).slice(0, 10); // Get the first 4 bytes
            const callData2= functionSelector2+ encodedData2.slice(2); // Remove '0x' prefix from encoded data
            const res=await modularComplianceImplementation.callModuleFunction(callData2,holdTimeCompliance.address);
        });
    });
    ...
    it('should return the correct name for the module', async function () {
        ...
        const moduleName = await holdTimeCompliance.name();
        expect(moduleName).to.equal('HoldTimeModule');
    });
    ...
    it('Revert Module Mint action if it is not owner', async function () { 
        ...
        await expect(modularComplianceImplementation.callModuleFunction(callData,holdTimeCompliance.address)).to.be.revertedWith('Ownable: caller is not the owner');
    });
